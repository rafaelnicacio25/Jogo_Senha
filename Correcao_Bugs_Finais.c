...
int main() {
    ...
    printf("\n~ Lily: O jogo funciona assim: eu tranquei o bau com uma senha magica de 4 digitos!\n");
    printf("Cada digito e um numero de 0 a 9. Voce chuta os 4 numeros separados por espaco.\n");
    printf("Eu vou te dizer quantos voce acertou na posicao certa, e quantos existem na senha mas tao na posicao errada!\n");
    ...
    while (!acertou) {
        ...
        printf("--------------------------------------------------\n");
        printf("Tentativa #%d\n", tentativas);
        printf("%s, digite seu palpite (4 numeros separados por espaco): ", nome_jogador);
        
        scanf("%d %d %d %d", &palpite[0], &palpite[1], &palpite[2], &palpite[3]);

        int posicao_certa = 0;
        int fora_posicao = 0;
      
        int senha_marcada[4] = {0, 0, 0, 0};
        int palpite_marcado[4] = {0, 0, 0, 0};
        ...
        for(int i = 0; i < 4; i++) {
            if(!palpite_marcado[i]) { 
                for(int j = 0; j < 4; j++) {
                    if(!senha_marcada[j] && palpite[i] == senha[j]) {
                        fora_posicao++;
                        senha_marcada[j] = 1; 
                        break; 
                    }
        ...
        printf("\n~ Lily: Existem %d numeros na posicao certa e %d numeros fora de posicao!\n", posicao_certa, fora_posicao);
        
        if(posicao_certa == 4) {
            printf("\n==================================================\n");
            printf("~ Narrador: *CLACK* O bau se abre violentamente!\n");
            printf("~ Lily: PARABENS %s!! Voce conseguiu em %d tentativas!\n", nome_jogador, tentativas);
            printf("O tesouro eh seu! Mas o melhor tesouro foi a nossa amizade <3\n");
            printf("==================================================\n");
            acertou = 1;
        }
    }
